...
time=[283458.5551376384,339787.25493184885,486441.7390473417,613668.1998813081,624619.702074532,702224.1417656919]
energy=[26186.883964647637,39331.95851733397,42784.57267866174,70026.15316575069,74775.00001,79523.8468537353]
cost=[150429405935.31754,261467923082.64163,313798051269.00726,361769326321.1023, 555234167404.6108,588276909532.2822]
caching=[0.5,1.0,1.5,2.0,2.5,3.0]
for i in range(0,len(caching)):
    time[i]=time[i]/20000
    energy[i]=energy[i]/1000
    cost[i]=cost[i]/(pow(10,12))
print("Time : ",time)
print("Cost : ",cost)
print("Energy : ",energy)
user_time=dict(zip(caching, time))
user_time=sorted(user_time.items(), key=operator.itemgetter(1))
user_energy=dict(zip(caching, energy))
user_energy=sorted(user_energy.items(), key=operator.itemgetter(1))
user_cost=dict(zip(caching, cost))
user_cost=sorted(user_cost.items(), key=operator.itemgetter(1))
print("Time ",user_time)
print("Energy ",user_energy)
print("Cost ",user_cost)
plt.plot(caching,time,label = "BMOGWO",marker='p')
plt.xlabel('Computation cycles per task')
# naming the y axis
plt.ylabel('Average Latency (ms)')
plt.legend()
plt.show()
plt.plot(caching,energy,label = "BMOGWO",marker='p')
plt.xlabel('Computation cycles per task')
# naming the y axis
plt.ylabel('Average Energy (J)')
plt.legend()
plt.show()
plt.plot(caching,cost,label = "BMOGWO",marker='p')
plt.xlabel('Computation cycles per task')
# naming the y axis
plt.ylabel('Average Normalized Cost')
plt.legend()
